...
export async function loadAll(teamCode, teamId) {
  const [game, stocks, positions, trades, hints, snaps, board, me, cash, bcast, fin, macro, optionsContracts, myOptions] =
    await Promise.all([
      select('game_state', '*'),
      select('stocks', '*'),
...
export async function refetchMine(teamCode, teamId) {
  const [positions, trades, hints, snaps, board, game, cash, bcast, fin, macro, stocks, optionsContracts, myOptions] =
    await Promise.all([
      select('positions', '*', (q) => q.eq('team_id', teamId)),
      select('trades', '*', (q) => q.eq('team_id', teamId).order('created_at', { ascending: false })),
      rpc('get_my_hints', { p_team_code: teamCode }),
