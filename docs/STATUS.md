...
> 파생/옵션 · 재무·힌트 자동 정합 · Gemini 문장 생성 · 관리자 통계 · QR 입장**을 얹었다(0033~0055, **구 프로덕션 DB 미적용**).
...
- 마이그레이션 **0054~0055**: `game_state.quick_join_enabled`, 관리자 토글 RPC, 랜덤 이름의 1인 조를 만드는
  `quick_join_team()` 추가. 게임 리셋 시 자동으로 닫는다. 관리자 [시스템] 화면에서 QR 표시·링크 복사·켜기/끄기 제공.
- 관리자가 켜면 이미 열려 있는 학생 로그인 화면에도 QR이 즉시 나타난다. PC는 QR 스캔, 모바일은
  **[이 기기에서 바로 입장]**으로 `/?join=quick`을 열어 입력 없이 입장하며 참가 코드는 해당 기기 localStorage에만 저장한다.
  비활성·대회 시작 후에는 새 조를 만들지 않고 일반 입장 화면에서 이유를 안내한다.
- 개발 DB에 0054~0055 적용. 관리자 켜기/끄기와 비활성 QR 안내, auth 단위 테스트 3개, 빌드 통과.
  현재 개발 게임이 R3라 데이터를 보존하기 위해 실제 신규 조 생성은 실행하지 않았고, 다음 게임 리셋 후
  실기기 QR 스캔 리허설이 남아 있다.

---

## 2026-09-18 모바일 개선 — 로컬 작업본

- 재검토 보정: 타이머 일시정지 때 차트가 사라지지 않고 정지 순간까지 공개된 경로를 유지하도록 수정. 확대 차트는 별도 방문 기록을 써서 휴대폰 뒤로가기가 확대만 닫고 종목 상세에 머문다. 컴포넌트와 모바일 방문 기록을 함께 붙인 회귀 테스트 추가, 390×844 실제 브라우저 확인.
- 그래프 가독성 추가 개선: 모바일 기본 플롯 높이 최소 260px, 굵은 가격선·확대 가격 숫자, 접을 수 있는 그리기 도구, [크게 보기] 모달. 390×844에서 기존 약 142px → 기본 260px / 확대 약 577px 확인. 320×568에서도 가로 넘침 없이 닫기·기간 변경·그리기 접근 가능. 확대 기간 선택은 원래 차트에 반영되고 기존 메모·공개 경로를 유지한다. 시스템 뒤로가면 확대 창도 닫힌다. 최종 Vitest **26파일 / 264개 통과**, 빌드 통과(기존 큰 청크 경고).

- 추가 요청: 휴대폰 시스템 뒤로가기/제스처를 History API에 연결. 주문 → 상세 → 목록 순서, 앞으로가기, 화면 복귀 버튼의 기록 재사용, 상품 모드 복원 및 빠른 탭 전환을 검증했다. 실제 브라우저에서 두 번의 뒤로가기 후 목록 표시와 주문 수량 유지 확인. 최종 Vitest **26파일 / 263개 통과**, 빌드 통과. 구현은 `src/useStudentNavigation.js`, 회귀 검증은 `src/useStudentNavigation.test.jsx`.

- 1023px 이하 회전 안내 제거. 학생 종목 검색·보유 필터, 상세 고정 매수/매도, 주문 상단 상세/목록 복귀, 예금·파생 모바일 패널 전환 구현. 패널을 유지해 검색어·수량·메모를 보존한다.
- 관리자 운영/순위 바로가기와 전체 11개 화면 선택, 라벨이 있는 재무·시황 폼, 카드 내 표 스크롤, 빠른 진행 패널의 화면 높이 제한 구현. 헤더 일시정지 타이머를 고정하고 모달을 body portal로 렌더한다.
- 검증: Vitest **25파일 / 258개 통과**, 배포 빌드 통과(기존 500kB 초과 청크 경고). 실제 브라우저 390×844·320×568에서 목록/검색/상세/매수/매도/복귀·주문 실패 수량 유지·체결 안내·내 자산 팝업·예금 해지 확인·옵션 화면·관리자 11개 화면 이동 및 일시정지 확인. 1024×768에서 기존 세 패널이 함께 표시되는 것을 확인했다.
- `/mobile-preview.html`은 개발 서버에서만 사용하는 예시 데이터 화면. 원격 DB 마이그레이션·게임 리셋·실제 주문·배포는 수행하지 않았다. 실제 휴대폰 키보드·터치와 교실 네트워크 리허설은 남아 있다.

## 2026-09-09 추가 점검 — 로컬 작업본

- UX: 단일가 모드 주문 예상가·최대수량을 종가로 통일. 현물·옵션·예금 가입 실패 시 입력 유지와 인라인 오류 안내. 예금 해지는 예상 수령액·소멸 이자 확인 후 실행.
- 관리자 빠른 패널: 일시정지 시 남은 시간 고정, 재개 RPC 사용, 타이머 재시작 시 게임 설정 시간 사용.
- 검증: Vitest **24파일 / 251개 통과**, 빌드 통과(500kB 초과 청크 경고). 실제 브라우저의 임시 컴포넌트 화면으로 고정가·실패 복구·해지 확인·빠른 재개 검증, 1024×768 다크 테마 배치 확인. 원격 RPC 실행을 포함한 통합 리허설은 하지 않음.
- 수학 검토: [주가 생성 검토](PRICE_MATH_REVIEW.md), 재현 스크립트 `scripts/verify-price-math.mjs`. 기본 확률식은 맞지만 0원 종목 복원·위기 변동성 역전·표준 GARCH와의 차이·업종 베타 무작위 배정 등 확인. **엔진 수식과 데이터셋은 변경하지 않음.**
- 아래 원격 DB 상태는 종전 기록이며 이번 작업에서 재확인하거나 마이그레이션하지 않았음.
...
| 마이그레이션 | **0001~0055 전부** | **0001~0032까지만** |
...
- **0033~0055의 기능(아래 §1-B)은 구 프로덕션 DB에 없다.** `main` push는 프론트만 재배포한다 —
  스키마는 `npx supabase db push`를 prod 대상으로 따로 돌려야 따라온다.
- **대회 전 필수** — prod에 0033~0055 push → 재시드 → `reset_game` → 실기기 리허설. §6-2 참고.
...
- **신규 독립 배포** — <https://cufic-wts-mobile.vercel.app> (`pepsigajoha/cufic-wts`, 현재 개발 DB 연결).
...
### 🟩 B. 개발 DB에만 (0033~0055 · 신규 Vercel 배포는 개발 DB 사용, **구 prod DB 미적용**)
...
| `StudentWorkspace.jsx` | 모바일 종목·상세·주문 전환, 상세 고정 거래 버튼, 명시적 목록·상세 복귀 |
...
> 아래는 **개발 DB(0001~0055 전부 적용) 기준**. 구 프로덕션은 §0·§1-A대로 0032 상태(8지표·옵션·장중 경로·QR 입장 없음).
...
- **프로덕션 DB에서의 0033~0055** — 개발 DB에서만 검증. prod push 후 재검증 필요.
...
- **개발 DB(`lynchogququuwihhixrd`)** — 0001~0055 적용. 0054~0055는 2026-09-19 `db push` 완료.
...
2. **프로덕션 DB가 0032에 멈춰 있다 (최우선).** 개발 DB는 0055. 대회 전:
   `.env`를 prod로 교체 → `npx supabase db push`(0033~0055) → `node scripts/gen-seed.mjs` 후 시드 재적용 →
