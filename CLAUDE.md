...
- **모바일 + 태블릿 + 노트북.** 1280px 이상 기준, 1024~1279px는 축소 레이아웃.
- **1023px 이하는 모바일 화면.** 회전 차단 없이 종목 → 상세 → 주문으로 이동한다. 주문 상단 [종목 상세로]·[종목 목록]으로 복귀한다. 관리자는 운영·순위 바로가기와 전체 화면 선택을 쓴다. 패널은 CSS로 전환하고 입력을 유지한다.
