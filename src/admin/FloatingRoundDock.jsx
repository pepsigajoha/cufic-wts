...
export default function FloatingRoundDock({ game, stocks, actions, notify, refresh, onNavigate }) {
  ...
  const quickGenerate = (preset) => {
    ...
    try {
      const nextPaths = simulateNextRound({ stockIds, currentPrices, macro, seed: QUICK_SEED, betaFx, betaOil, returnPath: true })
  ...
  const quickApply = async () => {
    ...
    const applied = await actions.applySimulatedPrices(
      quickPreview.applyPrices,
      quickPreview.paths ?? null,
      quickPreview.pathYear ?? null,
    )
    if (!applied.ok) {
      setQuickBusy(false)
