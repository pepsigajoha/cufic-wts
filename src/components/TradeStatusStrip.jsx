// 거래 상태 스트립 — "지금 매매할 수 있나?"를 한 문장으로. 헤더 아래 전폭.
// 왼쪽에 모드 전환(주식/파생) 버튼을 함께 얹어 두 줄을 한 줄로 합쳤다.
// state는 App이 이미 파생한 값(started·ended·timerState)만 조합해 넘긴다.
//   before / waiting / live / paused / closed / ended
...
export default function TradeStatusStrip({ state = 'before', children }) {
  ...
  return (
    ...
    <div className={'trade-strip ts-' + state} role="status">
      {children && <div className="ts-modes">{children}</div>}
